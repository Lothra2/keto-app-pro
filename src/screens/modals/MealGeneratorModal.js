  ...
  const handleGenerate = async () => {
    ...
    try {
      if (isFullDay) {
        ...
        const mealTarget = Math.round((baseDay.kcal || 1600) * (mealPercents[mealKey] || 0.3));
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {isFullDay && (result || baseDay) ? (
          <Card style={styles.dayCard}>
            ...
            <Text style={styles.daySubtitle}>
              {(result?.kcal || baseDay.kcal || 1600)} kcal · {result?.macros?.carbs || baseDay.macros?.carbs} C · {result?.macros?.prot || baseDay.macros?.prot} P · {result?.macros?.fat || baseDay.macros?.fat} G
            </Text>
            {['desayuno', 'almuerzo', 'cena'].map(key => {
        ...
        {loading ? (
          <View style={styles.loadingBox}>
            <LoadingSpinner label={language === 'en' ? 'Asking AI…' : 'Consultando IA…'} />
          </View>
        ) : (
          <Button
