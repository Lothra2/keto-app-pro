...
const clampDayIndex = (dayValue = 0, totalDays = 0) => {
...
const WorkoutScreen = ({ route, navigation }) => {
  ...
  const clampToPlanRange = useCallback(
    (value) => clampDayIndex(value, totalDays),
    [totalDays]
  );

  const activeDayIndex = useMemo(
    () => clampToPlanRange(typeof currentDay === 'number' ? currentDay : 0),
    [clampToPlanRange, currentDay]
  );
  const week = Math.floor(activeDayIndex / 7) + 1;

  useEffect(() => {
    const clamped = clampToPlanRange(currentDay);
    if (typeof currentDay !== 'number' || clamped !== currentDay) {
      setCurrentDay(clamped);
    }
  }, [currentDay, clampToPlanRange, setCurrentDay]);
  ...
  useEffect(() => {
    ...
    const clamped = clampToPlanRange(incomingDay);
    if (clamped !== activeDayIndex) {
      setCurrentDay(clamped);
    }
  }, [focusDay, dayIndex, clampToPlanRange, activeDayIndex, setCurrentDay]);
  ...
  const handleChangeDay = (direction) => {
    const next = clampToPlanRange(activeDayIndex + direction);
